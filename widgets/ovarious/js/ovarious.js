/*
    ioBroker.vis ovarious Widget-Set
...
"use strict";
/* jshint -W069 */
/* globals $,systemDictionary,vis,ejs,ace,_ */

// add translations for edit mode
fetch("widgets/ovarious/i18n/translations.json").then(async res => {
...
// this code can be placed directly in ovarious.html
vis.binds["ovarious"] = {
    version: "1.0.0",
    showVersion: function () {
        if (vis.binds["ovarious"].version) {
            console.log("Version ovarious: " + vis.binds["ovarious"].version);
            vis.binds["ovarious"].version = null;
        }
    },
    jsontemplate2: {
        createWidget: function (widgetID, view, data, style) {
            ...
            if (!$div.length) {
                return setTimeout(function () {
                    vis.binds["ovarious"].jsontemplate2.createWidget(widgetID, view, data, style);
                }, 100);
            }
            const bound = [];
            ...
            function onChange(e, newVal, oldVal) {
                if (newVal) vis.binds["ovarious"].jsontemplate2.createWidget(widgetID, view, data, style);
            }

            if (bound) {
                // eslint-disable-next-line no-constant-condition
                if (1 || !vis.editMode) {
                    vis.binds["ovarious"].bindStates($div, bound, vis.binds["ovarious"].jsontemplate2.onChange.bind({ widgetID: widgetID, view: view, data: data, style: style }));
                }
            }
            let text = "";
            try {
                ...
                text = vis.binds["ovarious"].escapeHTML(e.message).replace(/(?:\r\n|\r|\n)/g, "<br>");
                text = text.replace(/ /gm, "&nbsp;");
                text = '<code style="color:red;">' + text + "</code>";
            }
            if ((data.json_oid || "") == "") text += "No json datapoint<br>";
            if (template == "") text += "No template<br>";
            $("#" + widgetID).html(text);
        },
        // eslint-disable-next-line no-unused-vars
        onChange: function (e, newVal, oldVal) {
            if (newVal) vis.binds["ovarious"].jsontemplate2.render(this.widgetID, this.view, this.data, this.style);
        },
        // eslint-disable-next-line no-unused-vars
    ...
    editEjs: function (widAttr) {
        ...
        line.button = {
            ...
            click: function (/*event*/) {
                ...
                ace.config.setModuleUrl("ace/mode/ejs", "widgets/ovarious/js/mode-ejs.js");
...
vis.binds["ovarious"].showVersion();
